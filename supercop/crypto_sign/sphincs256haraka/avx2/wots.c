...
static void gen_chain_8x(unsigned char out[8*HASH_BYTES], const unsigned char seed[8*HASH_BYTES], const unsigned char *masks, int chainlen)
{
  int i,j;

  for(i=0;i<chainlen;i++)
  {
    hash_n_n_mask_8x(out,out,masks+(i*HASH_BYTES));
  }
}
...
static void gen_fullchains(unsigned char x[WOTS_L*HASH_BYTES], const unsigned char masks[(WOTS_W-1)*HASH_BYTES])
{
  int i;
  for(i=0;i<64;i+=8)
    gen_chain_8x(x+i*HASH_BYTES, x+i*HASH_BYTES, masks, WOTS_W-1);

  for(i=64;i<WOTS_L;i++)
    gen_chain(x+i*HASH_BYTES, x+i*HASH_BYTES, masks, WOTS_W-1); 
}
